...
PImage draw_mask()
{
  ...
  for (int i = 0; i < cnt_shapes; i++) {
    ...
    {
      /*
      imgMask.line(400 + (avg_radius + lat*sin(t + (j*j % cnt_sides)))*cos(t*speed + TWO_PI*j/cnt_sides),
                   400 + (avg_radius + lat*sin(t + (j*j % cnt_sides)))*sin(t*speed + TWO_PI*j/cnt_sides),
                   400 + (avg_radius + lat*sin(t + ((j+1)*(j+1) % cnt_sides)))*cos(t*speed + TWO_PI*(j+1)/cnt_sides),
                   400 + (avg_radius + lat*sin(t + ((j+1)*(j+1) % cnt_sides)))*sin(t*speed + TWO_PI*(j+1)/cnt_sides));
       */
      imgMask.line(400 + (avg_radius + lat*sin(t)*cos(TWO_PI*j/cnt_sides))*cos(t*speed + TWO_PI*j/cnt_sides),
                   400 + (avg_radius + lat*sin(t)*cos(TWO_PI*j/cnt_sides))*sin(t*speed + TWO_PI*j/cnt_sides),
                   400 + (avg_radius + lat*sin(t)*cos(TWO_PI*(j+1)/cnt_sides))*cos(t*speed + TWO_PI*(j+1)/cnt_sides),
                   400 + (avg_radius + lat*sin(t)*cos(TWO_PI*(j+1)/cnt_sides))*sin(t*speed + TWO_PI*(j+1)/cnt_sides));
